...
usage () {
    ...
    echo "       -p password    set password for root, default 'root'"
...
ULEN=12              # U-Boot Image lengths byte size
IMG1LEN=`(echo -n "ibase=16;obase=A;"; hexdump -s 64 -n 4 -ve '/1 "%02X"' $MK_BASEDIR/input/$FW_FILE ;echo) | bc`
IMG2LEN=`(echo -n "ibase=16;obase=A;"; hexdump -s 68 -n 4 -ve '/1 "%02X"' $MK_BASEDIR/input/$FW_FILE ;echo) | bc`
DATALEN=`echo $ULEN + $IMG1LEN + $IMG2LEN|bc`
HDRCRC32=`hexdump -s 4  -n 4 -ve '/1 "%02X"' $MK_BASEDIR/input/$FW_FILE`
DATACRC32=`hexdump -s 12 -n 4 -ve '/1 "%02X"' $MK_BASEDIR/input/$FW_FILE`
...
dd if=$MK_BASEDIR/input/$FW_FILE bs=1  skip=64                         count=12       of=$MK_BASEDIR/input/u02-len.dat   
if [ ! -e $MK_BASEDIR/input/u03-kern.dat ]
then
   dd if=$MK_BASEDIR/input/$FW_FILE bs=1  skip=76                         count=$IMG1LEN of=$MK_BASEDIR/input/u03-kern.dat
fi
if [ ! -e $MK_BASEDIR/input/u04-sqfs.dat ]
then
    dd if=$MK_BASEDIR/input/$FW_FILE bs=1  skip=`echo "76 + $IMG1LEN"|bc`  count=$IMG2LEN of=$MK_BASEDIR/input/u04-sqfs.dat
fi
...
fakeroot -i $MK_BASEDIR/fakeroot.dat -s $MK_BASEDIR/fakeroot.dat rsync --exclude=.gitignore -rav $MK_BASEDIR/root-overlay/bl400 $MK_BASEDIR/output/root/
...
echo -n $NEWIMG2LENHEX | xxd -r -p | dd of=$MK_BASEDIR/output/u02-len.dat conv=notrunc bs=1 count=4 seek=4
